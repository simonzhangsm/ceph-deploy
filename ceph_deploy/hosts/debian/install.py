...
def install(distro, version_kind, version, adjust_repos, **kw):
    ...
    if adjust_repos:
        # Wheezy does not like the mirrors.ustc.edu.cn/ceph SSL cert
        protocol = 'https'
        if codename == 'wheezy':
            protocol = 'http'
        ...
        if version_kind in ['dev', 'dev_commit']:
            ...
            if version_kind == 'stable':
                url = '{protocol}://mirrors.ustc.edu.cn/ceph/debian-{version}/'.format(
                    protocol=protocol,
                    version=version,
                    )
            elif version_kind == 'testing':
                url = '{protocol}://mirrors.ustc.edu.cn/ceph/debian-testing/'.format(
                    protocol=protocol,
                    )
            else:
